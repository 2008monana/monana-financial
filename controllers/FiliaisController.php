...
class FiliaisController extends Controller
{
    ...
    public function alternarEstado(string $id): void
    {
        ...
        if ($filial) {
            $novoEstado = $filial['ativa'] ? 0 : 1;
            $this->filialModel->atualizar((int) $id, ['ativa' => $novoEstado]);
            AuditoriaHelper::registar($novoEstado ? 'filial_ativada' : 'filial_desativada', 'filiais', (int) $id, ['ativa' => $filial['ativa']], ['ativa' => $novoEstado], $novoEstado ? 'baixa' : 'media');
