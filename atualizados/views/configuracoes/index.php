<?php if (!empty($modoImpressao)): ?><table><thead><tr><th>Data</th><th>Utilizador</th><th>Módulo</th><th>Descrição</th><th>IP</th></tr></thead><tbody><?php foreach($logs as $log): ?><tr><td><?=htmlspecialchars($log['criado_em'])?></td><td><?=htmlspecialchars($log['usuario_nome']??'Sistema')?></td><td><?=htmlspecialchars(AuditoriaHelper::moduloDeAcao($log['acao'],$log['tabela_afetada']??null))?></td><td><?=htmlspecialchars(AuditoriaHelper::descricaoEvento($log))?></td><td><?=htmlspecialchars($log['ip_origem']??'')?></td></tr><?php endforeach?></tbody></table><?php return; endif; ?>
...
.selo-modulo{background:rgba(100,116,139,.12);color:#475569}
.celula-descricao{max-width:340px}
.celula-descricao .descricao-extra{display:block;color:var(--muted);font-size:12px;margin-top:3px;white-space:normal}
